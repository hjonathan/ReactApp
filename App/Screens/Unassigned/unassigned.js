...
class Unassigned extends Component {
    ...
    static navigationOptions = ({navigation}) => {
        ...
        return {
            title: I18n.t("menu_drawer_main_menu_item_unassigned"),
     ...
     */
    componentWillMount () {
        this.fnList.getDataList("requestCasesUnassigned");
    }
    ...
    componentWillReceiveProps (nextProps) {
        try {
            if (!nextProps.cases.unassigned.length) {
                this.setState({loading: false});
            }
            if (Object.prototype.hasOwnProperty.call(nextProps.cases, "unassigned") &&
                nextProps.cases.unassigned && !this.state.unassignedRequested) {
                this.setState({unassignedRequested: true, data: []});
                this.fnList.getUsersInfo(nextProps.cases.unassigned, "prevUser");
            }
            if (Object.prototype.hasOwnProperty.call(nextProps.cases, "users") &&
                ...
                this.setState({
                    data: this.fnList.mergeList({
                        caseList: nextProps.cases.unassigned,
    ...
    onRefresh (event) {
        ...
        if (this.props.isConnected) {
            this.props.reset();
            this.props.routeName(this.props.navigation.state.routeName);
            this.setState({unassignedRequested: false, usersRequested: false});
            this.fnList.getDataList("requestCasesUnassigned", {search});
        }
    }
