...
class Inbox extends Component {
    ...
    static navigationOptions = ({navigation}) => {
        ...
        return {
            title: I18n.t("menu_drawer_main_menu_item_inbox"),
     ...
     */
    componentWillMount () {
        this.fnList.getDataList("casesTodoRequest");
    }
    ...
    componentWillReceiveProps (nextProps) {
        try {
            if (!nextProps.cases.todo.response.length) {
                this.setState({loading: false});
            }
            if (Object.prototype.hasOwnProperty.call(nextProps.cases, "todo") &&
                nextProps.cases.todo.response && !this.state.todoRequested) {
                this.setState({todoRequested: true, data: []});
                this.fnList.getUsersInfo(nextProps.cases.todo.response, "prevUser");
            }
            if (Object.prototype.hasOwnProperty.call(nextProps.cases, "users") &&
                ...
                this.setState({
                    data: this.fnList.mergeList({
                        caseList: nextProps.cases.todo.response,
    ...
    onRefresh (event) {
        ...
        if (this.props.isConnected) {
            this.props.reset();
            this.props.routeName(this.props.navigation.state.routeName);
            this.setState({todoRequested: false, usersRequested: false});
            this.fnList.getDataList("casesTodoRequest", {search});
        }
    }
